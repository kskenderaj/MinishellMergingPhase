...
/* Basics --Begin */
int							outfile_redirector(t_file_node *file_node, t_shell_state *shell);
int							infile_redirector(t_file_node *file_node, t_shell_state *shell);
void						dup_and_or_close(int *prev_in_out, int *new_in_out, t_shell_state *shell);
void						close_and_reset(int *prev_in_out, int *new_in_out,
								int *reset, t_shell_state *shell);
int							pipe_handler(t_cmd_node *command, t_shell_state *shell);
/* Basics --End */
/* Redirectories-- BEGIN */
...
int							exec_builtin_with_redir(int (*builtin)(char **, t_shell_state *),
								char **args, int in_fd, int out_fd, t_shell_state *shell);
int							exec_heredoc(const char *delimiter, int quoted, t_shell_state *shell);
int							exec_heredoc_from_content(char *content,
								int quoted, t_shell_state *shell);
char						*expand_heredoc_line(char *line, int should_expand, t_shell_state *shell);
...
int							handle_separated_operators(char **args, int *i,
								int *in_fd, int *out_fd, t_shell_state *shell);
pid_t						exec_parent_runner(t_cmd_node *cmd, int *io_data, t_shell_state *shell);
/* Main utilities */
...
int							calculate_word_length(t_parse_state *state,
								char delim);
char						*allocate_result_buffer(t_parse_state *state, t_shell_state *shell);
...
int							handling_the_infile(t_commandlist *cmd,
								t_filelist *current, int update, t_shell_state *shell);
