...
/*   Updated: 2025/11/23 16:41:56 by kskender         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	setup_child_io_and_exec(t_child_ctx *ctx, t_shell_state *shell)
{
	...
	if (!ctx->cmd || !ctx->cmd[0] || !ctx->cmd[0][0])
		exit(0);
	if (!strcmp(ctx->cmd[0], "echo") || !strcmp(ctx->cmd[0], "cd")
		|| !strcmp(ctx->cmd[0], "pwd") || !strcmp(ctx->cmd[0], "export")
		|| !strcmp(ctx->cmd[0], "unset") || !strcmp(ctx->cmd[0], "env")
		|| !strcmp(ctx->cmd[0], "exit"))
	...
	t_child_ctx *child, t_cmd_node *node, t_shell_state *shell)
{
	...
	fds[0] = setup_input_file_from_cmd(node, shell);
	fds[1] = setup_output_file_from_cmd(node, shell);
	init.child = child;
	init.ctx = ctx;
	init.i = i;
	init.in_fd = fds[0];
	...
	if (ctx->pids[i] == 0)
	{
		...
		setup_child_io_and_exec(child, shell);
	}
	close_fds(fds[0], fds[1], shell);
	return (0);
}
...
int	spawn_pipeline_children(t_pipeline_ctx *ctx, t_shell_state *shell)
{
	...
	while (i < ctx->ncmds && node)
	{
		if (setup_and_fork_child(ctx, i, &child, node, shell) != 0)
			return (1);
		node = node->next;
		i++;
	}
