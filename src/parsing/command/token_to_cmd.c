...
/*   Updated: 2025/11/21 13:07:55 by jtoumani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**create_array(t_token *token, t_cmd_node *cmdnode, int i)
{
	...
	if (!cmd_array)
		return (NULL);
	if (process_tokens_to_array(token, cmdnode, cmd_array, &i) < 0)
		return (NULL);
	cmd_array[i] = NULL;
	return (cmd_array);
}
...
void	final_token(t_token_list *toklst, t_env_list *envlst, int last_status)
{
	...
	while (token)
	{
		if (process_single_token(token, &skip_next, envlst, last_status) < 0)
			return ;
		token = token->next;
	}
}
